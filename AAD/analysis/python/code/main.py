...
for sb in range(cfg.sbnum):
    ...
    if cfg.model_name == 'CNN_baseline':
        ...
        datasize = eegdata.shape
        eegdata = eegdata.reshape(4, 10, datasize[1], datasize[2], datasize[3])
        eeglabel = eeglabel.reshape(4, 10, datasize[1])
        # trans 10 and 4
        eegdata = np.transpose(eegdata, (1, 0, 2, 3, 4))
        eeglabel = np.transpose(eeglabel, (1, 0, 2))
    ...
    for fold, (train_ids,  test_ids) in enumerate(kfold.split(eegdata)):
        ...
        if cfg.model_name == 'CNN_baseline':
            traindata = traindata.reshape(32 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window, 64)
            trainlabel = trainlabel.reshape(32 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window)
            testdata = testdata.reshape(8 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window, 64)
            testlabel = testlabel.reshape(8 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window)
        else:
            traindata = traindata.reshape(32 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window, 9,
                                          9)
            trainlabel = trainlabel.reshape(32 * int(60 * cfg.sample_rate / cfg.decision_window), cfg.decision_window)
