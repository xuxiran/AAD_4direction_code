function [r,p,mse,pred,model,c] = mTRFcrossval(stim,resp,fs,map,tmin,tmax,lambda)
...
% edited by FZ, 28-10-2018
c = model(:,:,1:size(x{1},2),:);
if map ==  -1
    model = reshape(model(:,:,size(x{1},2)+1:end,:),size(model,1),size(model,2),size(x{1},2),length(tmin:tmax));
else
    model = reshape(model(:,:,size(x{1},2)+1:end,:),size(model,1),size(model,2),length(tmin:tmax),size(y{1},2));
    c = squeeze(c);
end
end